"""
Ultra Flash: Cascaded Streaming Pipeline
SF (LR causal generator) -> Latent Upsampler -> Sparse SR DiT -> HR Decoder
Streaming cascade: every N SF chunks are sent to the SR pipeline.
"""
...
class CausalCascadeStreamingPipeline:
    ...
    def _run_sr_for_group(
        ...
        from configs.causal_dmd_latentSR.train_causal_dmd import (
            _stream_forward_causal,
            _chunk_indices,
        )
        ...
        if is_first_group:
            ...
            out_list, sr_cache_state = _stream_forward_causal(
                ...
                kv_len=self.sr_kv_len,
                cache_state=sr_cache_state,
            )
            flow_pred = torch.stack(out_list, dim=0)
        else:
            slices = _chunk_indices(t_lat, self.sr_chunk_size)
            preds = []
            for sl in slices:
                x_chunk = latents[:, :, sl, :, :]
                y_chunk = cond_y_bcthw[:, :, sl, :, :]
                x_list = [x_chunk[i] for i in range(b)]
                y_list = [y_chunk[i] for i in range(b)]
                temporal_offset = (global_latent_offset + (sl.start or 0)) // patch_t
                out_list, sr_cache_state = _stream_forward_causal(
                    model=self.sr_dit,
                    x_list=x_list, t=t_val, context=prompt_context,
                    y_list=y_list,
                ...
                preds.append(torch.stack(out_list, dim=0))
            flow_pred = torch.cat(preds, dim=2)
    ...
    def inference(
        ...
        for current_num_frames in all_num_frames:
            ...
            for index, current_timestep in enumerate(sf.denoising_step_list):
                timestep = torch.ones(
                ...
                if index < len(sf.denoising_step_list) - 1:
                    next_t = sf.denoising_step_list[index + 1]
            ...
            print(f"[SF chunk {_sf_chunk_idx}] {current_num_frames} lat frames ({pix_frames} pix) | "
                  f"{_t_sf_chunk:.2f}s | {sf_fps:.1f} pix fps")
            _sf_chunk_idx += 1
