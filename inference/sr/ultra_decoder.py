...
class PixelShuffleUp(nn.Module):
    def __init__(self, n_in: int, n_out: int, scale: int = 2) -> None:
        super().__init__()
        self.up = nn.Sequential(
            conv(n_in, n_out * scale * scale),
    ...
    if parallel:
        ...
        for block in tqdm(model, disable=not show_progress_bar):
            if isinstance(block, MemBlock):
                ...
                mem = torch.cat([x_view[:, :1], x_view[:, :-1]], dim=1).reshape(x.shape)
                x = block(x, mem)
            else:
                x = block(x)
        nt, c, h, w = x.shape
...
class UltraDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.Sequential(
            ...
            PixelShuffleUp(n_f[0], n_f[0]) if decoder_space_upscale[0] else nn.Identity(),
            TGrow(n_f[0], 1),
            conv(n_f[0], n_f[1], bias=False),
            MemBlock(n_f[1], n_f[1]), MemBlock(n_f[1], n_f[1]), MemBlock(n_f[1], n_f[1]),
            PixelShuffleUp(n_f[1], n_f[1]) if decoder_space_upscale[1] else nn.Identity(),
            TGrow(n_f[1], 2 if decoder_time_upscale[0] else 1),
            conv(n_f[1], n_f[2], bias=False),
            MemBlock(n_f[2], n_f[2]), MemBlock(n_f[2], n_f[2]), MemBlock(n_f[2], n_f[2]),
            PixelShuffleUp(n_f[2], n_f[2]) if decoder_space_upscale[2] else nn.Identity(),
...
class WanUltraDecoder(nn.Module):
    """
    UltraForcing 自研 tiny decoder，使用 PixelShuffle 替代最近邻上采样。
    接口与 WanTinyDecoder 保持一致：
    - decode_latents: BCTHW latent -> BCTHW pixel[-1, 1]
    """

    def __init__(self, checkpoint_path: str, dtype: torch.dtype = torch.float16) -> None:
        super().__init__()
        self.model = UltraDecoder(checkpoint_path=checkpoint_path)
