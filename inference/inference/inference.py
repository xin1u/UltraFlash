...
class LightweightIQAEvaluator:
    ...
    def __init__(self, device: torch.device, threshold: float = 0.62,
        ...
        if taehv_ckpt:
            ...
            self._tiny_dec = TAEHV(checkpoint_path=taehv_ckpt).to(torch.float16).to(device).eval()
        else:
            self._tiny_dec = None
...
class DCMSparseCascadePipeline(CausalCascadeStreamingPipeline):
    ...
    def __init__(self, *a,
                 dcm_lr_steps_subsequent: int = 3,
                 dcm_adaptive_refresh: bool = True,
...
def parse_args():
    ...
    # HR Decoder
    parser.add_argument("--taehv", action="store_true", default=True)
    parser.add_argument("--taehv_ckpt", type=str, default="checkpoints/taehv.pth")
    # DCM params
    parser.add_argument("--dcm_lr_steps_subsequent", type=int, default=3)
    parser.add_argument("--dcm_adaptive_refresh", action="store_true", default=True)
...
def main():
    ...
    # ---- TAEHV HR Decoder ----
    if args.taehv:
        from utils.taehv import TAEHV, MemBlock

        class TAEHVWrapper(torch.nn.Module):
            def __init__(self, ckpt_path, dtype=torch.float16):
                super().__init__()
                self.taehv = TAEHV(checkpoint_path=ckpt_path).to(dtype)
                self._dtype = dtype
            ...
            def parameters(self, recurse=True):
                return self.taehv.parameters(recurse=recurse)
            ...
            def decode_to_pixel(self, x, use_cache=False, **kwargs):
                ...
                pixels = self.taehv.decode_video(x, parallel=True, show_progress_bar=False)
                pixels = pixels.mul_(2).sub_(1).float().clamp_(-1, 1)
                return pixels

        pipeline.vae = TAEHVWrapper(args.taehv_ckpt).eval().requires_grad_(False).to(device)
        print(f"[TAEHV] HR Decoder loaded: {args.taehv_ckpt}")
    ...
    if dcm_enabled and args.dcm_adaptive_refresh and not args.lr_only:
        try:
            iqa_evaluator = LightweightIQAEvaluator(
                ...
                taehv_ckpt=args.taehv_ckpt if args.taehv else None,
            )
