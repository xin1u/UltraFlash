...
Extracted from UltraForcing training configs for standalone inference.
"""
import math
...
def _compute_sparse_params(model: nn.Module, grid_sizes: torch.Tensor):
    """
    Compute block-sparse attention parameters based on model config and current resolution.
    Returns (top_k, local_num, local_range) for the sparse attention blocks.
    """
    top_k_eff = None
...
def _stream_forward_causal(
    ...
    cache_state: Optional[list[dict]] = None,
) -> tuple[list[torch.Tensor], list[dict]]:
    """
    Sparse causal DiT streaming forward pass.
    Processes chunks with KV cache and block-sparse attention.
    """
